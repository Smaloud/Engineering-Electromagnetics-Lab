...
for it_y = 1 : sampling_number_y
    for it_z = 1 : sampling_number_z
        % Obtain the Vector in yz plane
         P = [0, ...
            (it_y - 1) / sampling_density - length_y / 2, ... 
            (it_z - 1) / sampling_density - length_z / 2];
         % Iterate the current Vector
         for S_z = [-d / 2 , d / 2]
             % Iterate the angle segments
...
saveas(1, "Magnetic Field Intensity Vector Distribution","png");
...
saveas(2, " Magnetic Field Intensity Magnitude Distribution" , "png");
...
xlabel("y (m)"), ylabel("z (m)");
saveas(3, "Magnetic Line Distribution " , "png");
